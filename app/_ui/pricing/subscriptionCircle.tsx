...
export default function SubscriptionCircle({ remaining_messages, message_quota, daysRemaining, daysTotal }: SubscriptionCircleProps) {
  const daysPercent = (daysRemaining / daysTotal) * 100
  const messagePercent = message_quota > 0 ? (remaining_messages / message_quota) * 100 : 0

  return (
    <div className="relative w-2/3 max-w-lg mt-8">
      <svg viewBox="0 0 200 120" className="size-full">
        {/* پس‌زمینه بیرونی - پیام‌ها */}
        ...
        <motion.path
          ...
          strokeDashoffset={251.2 - (251.2 * messagePercent) / 100}
          initial={{ strokeDashoffset: 251.2 }}
          animate={{ strokeDashoffset: 251.2 - (251.2 * messagePercent) / 100 }}
      ...
      <p className="text-xl lg:text-3xl fill-neutral-700 dark:fill-neutral-300 font-semibold text-center mt-3">
        <AnimatedNumber value={remaining_messages} /> / {toPersianNumber(message_quota)} <span className="mx-2">پیام</span>
      </p>
    </div>
  )
}
